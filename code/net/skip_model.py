...
def unet(
        ...
        filter_size_up=3, filter_skip_size=1, need_sigmoid=False, need_relu = True, need_bias=True,
...
def sound_rec(num_input_channels=8, num_output_channels=1, pad='zero'):
    model = nn.Sequential()
    model.add(conv(num_input_channels, num_output_channels, 1, bias=True, pad=pad))
    model.add(nn.ReLU())
    return model
...
def skip_mask_vec(
        ...
        num_channels_up=[16, 32, 64, 128, 128],
        num_channels_skip=[4, 4, 4, 4, 4], filter_size_down=3,
        filter_size_up=3, filter_skip_size=1, need_bias=True,
        pad='zero', upsample_mode='nearest', downsample_mode='stride', act_fun='LeakyReLU', need1x1_up=True):
    """
    Assembles encoder-decoder with skip connections.
